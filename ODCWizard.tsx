...
export const ODCWizard: React.FC<ODCWizardProps> = ({
  ...
  const [destinationId, setDestinationId] = useState(LOCATOR_POINTS[1].id);
  const [destinationIds, setDestinationIds] = useState<string[]>([LOCATOR_POINTS[1].id]);
      ...
      {step === 1 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-2xs space-y-6 animate-in fade-in duration-150">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-xs">
            ...
            <div className="space-y-1.5">
              <label className="font-bold text-slate-700 block flex items-center justify-between">
                <span>Destination Shipment Locator</span>
                <span className="text-emerald-600 font-normal text-[11px]">GPS Verified</span>
              </label>
              <select
                value={destinationId}
                onChange={(e) => {
                  ...
                  setDestinationIds((current) => current.includes(e.target.value) ? current : [...current, e.target.value]);
                }}
                className="w-full p-2.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-800 font-medium focus:bg-white focus:border-blue-500 outline-none"
              >
                {LOCATOR_POINTS.map((loc) => (
              ...
              <div className="flex flex-wrap gap-1.5 pt-1">
                {destinationIds.map((id, index) => {
                  const location = LOCATOR_POINTS.find((loc) => loc.id === id);
                  return location ? (
                    <span key={id} className="px-2 py-1 rounded-lg bg-amber-50 border border-amber-200 text-[10px] font-semibold text-amber-900 flex items-center gap-1">
                      {index + 1}. {location.name} <span className="text-amber-600">({location.locatorRole || 'DESTINATION'})</span>
                      <button type="button" aria-label={`Remove ${location.name}`} onClick={() => setDestinationIds((current) => current.filter((item) => item !== id))} className="text-amber-700 hover:text-rose-600 font-bold">×</button>
                    </span>
                  ) : null;
                })}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-2xs space-y-6 animate-in fade-in duration-150">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              {automatedActivities.map((activity) => (
                <div key={activity.id} className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1.5">
                  ...
                  <p className="text-[10px] text-emerald-700 font-semibold">Geofence / system event • {activity.defaultCostItemName || 'Rule driver'}</p>
                </div>
              ))}
            </div>
          </div>
